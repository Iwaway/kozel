...
const { authValidate, registerValidate, activateAccountValidate, chatMessageValidate } = require('./utils/validator');
...
const httpRoutes = {
    auth: async (res, data) =>{
        ...
        if(!captchaResult) return res.end(JSON.stringify({ status: 'captcha-error', message: 'Incorrect captcha.' }))
